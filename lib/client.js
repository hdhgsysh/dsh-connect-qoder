window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const QODER_CARD_CSS = [
			...
			// Account panel: one row per region — the state badge, the identity
			// and its source, and the per-region confirm control. The frame
			// mirrors the usage panel's so the two read as one family.
			...
			".dsm-qoder-state-badge{font-size:10px;padding:1px 6px;border-radius:999px;border:1px solid var(--dsw-alias-border-l2,#36373b);color:var(--dsw-alias-label-primary,#1a1a1a);white-space:nowrap;flex:none}",
			".dsm-qoder-state-badge-ok{border-color:var(--dsw-alias-state-success-primary,#12b76a);color:var(--dsw-alias-state-success-primary,#12b76a)}",
			".dsm-qoder-state-badge-expired{border-color:var(--dsw-alias-state-error-primary,#d92d20);color:var(--dsw-alias-state-error-primary,#d92d20)}",
			".dsm-qoder-state-badge-needs{border-color:var(--dsw-alias-state-warning-primary,#f79009);color:var(--dsw-alias-state-warning-primary,#f79009)}",
			...
			".dsm-qoder-account-row-off .dsm-qoder-state-badge,.dsm-qoder-account-row-off .dsm-qoder-usage-region,.dsm-qoder-account-row-off .dsm-qoder-account-id{opacity:.55}",
		].join("");
		 ...
		 */
		function QoderUsagePanel({ t, refreshToken = 0 }) {
		...
		/**
		 * The account section: one row per region — which sign-in drives it,
		 * in which state that sign-in is, and what to do when it is not `ok`.
		 *
		 * The states come from the host's account route and are computed from
		 * LOCAL evidence only, so reading the panel costs no network. The two
		 * buttons are the only deliberate actions:
		 *
		 ...
		 */
		function QoderAccountPanel({ t, onReconciled, settingsScope }) {
			...
			// an app is not a fault of this machine.
			const badgeClassOf = (state) => state === "ok" ? " dsm-qoder-state-badge-ok" : state === "expired" ? " dsm-qoder-state-badge-expired" : state === "needs-app" ? " dsm-qoder-state-badge-needs" : "";
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					status === "error" ? (0, react_jsx_runtime.jsx)("p", {
						...
						const result = confirmState[entry.region];
						const hasIdentity = entry.identity !== undefined && entry.identity !== null;
						const name = hasIdentity ? String(entry.identity.name ?? "").trim() : "";
						const meta = [];
						if (entry.source === "env-pat") meta.push(t("account.envPat"));
						else if (typeof entry.appName === "string" && entry.appName !== "") meta.push(t("account.appFrom", { app: entry.appName }));
						if (hasIdentity && Number(entry.identity.expiresAt) > 0) meta.push(withDate(t("account.expiresAt"), entry.identity.expiresAt));
						// The note line: what the state MEANS for the user. A
						// manage-page link accompanies every state the card
						// cannot fix by itself.
						const manageLink = typeof entry.manageUrl === "string" && entry.manageUrl !== "" ? (0, react_jsx_runtime.jsx)("a", {
							href: entry.manageUrl,
							target: "_blank",
							rel: "noreferrer",
							children: ` · ${t("account.openManage")}`
						}) : null;
						// Provider switch state. The local map is the source of
						// truth for the switch (it settles on the host value
						// after each save); a region with no key yet reads as
						// offered, the same default the host predicate uses.
						const offered = enabledRegions[entry.region] !== false;
						return (0, react_jsx_runtime.jsxs)(react.Fragment, {
							children: [
								(0, react_jsx_runtime.jsxs)("div", {
									className: `dsm-qoder-account-row${offered ? "" : " dsm-qoder-account-row-off"}`,
									children: [
										(0, react_jsx_runtime.jsx)("span", {
											className: `dsm-qoder-state-badge${badgeClassOf(entry.state)}`,
											children: stateLabelOf(entry)
										}),
										(0, react_jsx_runtime.jsx)("span", {
											className: "dsm-qoder-usage-region",
										...
										(0, react_jsx_runtime.jsxs)("span", {
											...
											children: [
												...
												meta.length > 0 ? (0, react_jsx_runtime.jsx)("span", {
										...
										(0, react_jsx_runtime.jsx)("span", { className: "dsm-qoder-usage-spacer" }),
										entry.source !== undefined ? (0, react_jsx_runtime.jsx)("button", {
											type: "button",
											className: "dsm-qoder-button",
											disabled: confirmBusy[entry.region] === true,
											onClick: () => {
												void confirm(entry.region);
											},
											children: confirmBusy[entry.region] === true ? t("account.confirming") : t("account.confirm")
										}) : null
									]
								}),
								!offered ? (0, react_jsx_runtime.jsx)("p", {
									className: "dsm-qoder-account-note",
									children: t("account.offerOff")
								}) : null,
								entry.state === "needs-app" ? (0, react_jsx_runtime.jsxs)("p", {
									className: "dsm-qoder-account-note dsm-qoder-account-note-error",
									children: [
										t("account.readFail", { detail: entry.detail ?? "" }),
										manageLink
									]
								}) : null,
								entry.state === "expired" ? (0, react_jsx_runtime.jsxs)("p", {
									className: "dsm-qoder-account-note",
									children: [
								...
								entry.state === "signed-out" ? (0, react_jsx_runtime.jsxs)("p", {
									className: "dsm-qoder-account-note",
									children: [
										t("account.unsigned"),
										manageLink
								...
								result?.kind === "confirmed" ? (0, react_jsx_runtime.jsx)("p", {
									className: "dsm-qoder-account-note",
									children: t("account.confirmed")
								}) : null,
								result?.kind === "sign-in-expired" ? (0, react_jsx_runtime.jsxs)("p", {
									className: "dsm-qoder-account-note dsm-qoder-account-note-error",
									children: [
								...
								result?.kind === "unavailable" ? (0, react_jsx_runtime.jsx)("p", {
									className: "dsm-qoder-account-note dsm-qoder-account-note-error",
									children: t("account.confirmFailed", { detail: result.detail ?? "" })
								}) : null
							]
						}, `account:${entry.region ?? index}`);
					}),
