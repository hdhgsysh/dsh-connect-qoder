window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function QoderPluginCard({ t, settingsScope, view }) {
			...
			// Mirror dsh-connect-workbuddy: the host renders the card with
			// `view: "page"` on the bundle / row detail pages and
			// `view: "summary"` in list contexts, so open by default wherever the
			// host puts the card on a detail page. Legacy slots that pass no
			// `view` keep the previous collapsed default.
