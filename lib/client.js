window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const zh = {
			...
			"row.maxWindowTitle": "开启时按模型目录声明的最大窗口；关闭时使用 Qoder 客户端默认窗口（多数模型约 200K）。",
			"row.save": "保存",
			"row.saving": "保存中…",
			"row.saved": "已保存",
			"row.discard": "撤销更改",
			...
			"row.showHint": "未勾选的模型不会出现在模型选择器里。",
			"row.enableAll": "全部显示",
			"row.disableAll": "全部隐藏",
		...
		const en = {
			...
			"row.maxWindow": "Use the maximum context window declared by the model",
			"row.maxWindowTitle": "On: the declared maximum window is used. Off: the Qoder client default is used.",
			"row.save": "Save",
			"row.saving": "Saving…",
			"row.saved": "Saved",
		...
		function QoderAccountPanel({ t, onReconciled, settingsScope, activeRegion = "qoder-cn", onRegionChange }) {
			...
			return (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					reloadedAt !== undefined && !reloading ? (0, react_jsx_runtime.jsx)("p", {
						className: "dsm-qoder-state",
