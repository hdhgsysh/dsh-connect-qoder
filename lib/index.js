...
async function activate(ctx, config) {
  ...
  //
  // The holder itself is captured through a `current()` indirection — the same
  // shape WorkBuddy uses — because on DSH 0.1.7 the section can hand the plugin
  // a LIVE source (a `() => T` closure or a `{ get(): T }` reference to the
  // settings document) instead of a frozen value. Reading `preferences` directly
  // after such a hand-off would keep the startup copy forever; `current()`
  // resolves the live source at read time.
  let preferences = { ...config }
  let preferencesSource = () => preferences
  ...
  ctx.inject(['settings'], (settingsCtx) => {
    ...
    try {
      ...
      //
      // FIX 0.1.7 live-config: on the rewrite line the host hands the plugin a
      // LIVE source of its config values — `setSource` is the seam where that
      // happens on the installSection path, and `loader/volatile-update` is the
      // event the host dispatches after a settings write replays the fiber.
      // Both are what make a card save reach the running adapter without a DSH
      // restart: `current()` (above) resolves whatever `setSource` installed, and
      // the refresh re-snapshots the model list and tells the picker to drop it.
      // Older hosts keep working through the installSection callbacks; hosts
      // that offer neither simply keep their startup values until a restart.
      if (typeof settings.configure === 'function') {
        // On 0.1.7 the auto-form replays the config through
        // `loader/volatile-update` (subscribed below) and the settings service
        // owns the document — the plugin gets no `setSource`/`onChange`
        // callbacks on this path, so the live source is resolved lazily by
        // `current()` instead.
  ...
  // settings write replays the config — that is the live seam the configure()
  // auto-form relies on. Re-read the live source and refresh the picker on it.
  // Optional chaining: hosts before 0.1.7 have no such event (or no `ctx.on`
  // at all), and there the installSection callbacks above already do this.
        ...
        webCtx.webServer.register({
          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              if (row === undefined) {
                // The settings service stores a plugin's fields under whichever
                // namespace it registered; when that row is missing the write
                // cannot be persisted and the error names the namespaces it
                // actually saw, so a mis-registered row shows up here.
                return sendJson(res, 503, {
                  error: `${QODER_SETTINGS_NS} namespace missing from describe(); known: ${rows
                    .map((entry) => String(entry.ns))
                    .join(', ')}`,
                })
              }
              const incoming = body.value
              // On schemastery 3.18.4 the Host marks 0.1.7-rewritten plugin
              // values as volatile and hands them out as live `{ get() }`
              // references: expanding one (`{ ...live() }`) leaks the `get`
              // function into the mutate value and the document's strict JSON
              // check rejects it. Every read from `row.value` unwraps.
              const live = () => {
              ...
              let merged
              if (shape === 'regions') {
                // Keep every region's allow-list: merge the incoming keys in,
                // never delete a sibling region the request does not touch.
                const stored = live()
              ...
              // Re-read through the service (describe() projects the document
              // fresh on every call, so this is a post-mutate read): this is
              // the check that turns a "reported success" into "did actually
              // persist", because the settings scope's own snapshot can trail
              // the document after a 0.1.7 write.
