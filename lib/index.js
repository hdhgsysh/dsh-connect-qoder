...
import { normalizeEntry, projectModelRow } from './catalog-entry.js'
        ...
        handler: async (req, res) => {
          ...
          for (const { runtime } of started) {
            for (const entry of runtime.catalog.current()) {
              models.push(projectModelRow(entry, runtime.region, now, MODEL_RATES))
            }
          }
          // Each of these is unwrapped individually. `current()` resolves the
          // SOURCE, but a field inside a resolved document can itself be a
          // 0.1.7 volatile `{ get() }` shell — and `typeof shell === 'object'`
          // is true, so the guards below would have passed it straight through
          // to the card as an object with no overrides in it.
          const settings = current()
          const overrides = unwrapReference(settings.imageOverrides)
          const enabled = unwrapReference(settings.enabledModelIds)
          sendJson(res, 200, {
            models,
            imageOverrides: overrides !== null && typeof overrides === 'object' ? overrides : {},
            useMaximumContextWindow: unwrapReference(settings.useMaximumContextWindow) === true,
