          ...
          handler: async (req, res) => {
            ...
            try {
              ...
              const row =
                ...
                rows.find((entry) => String(entry.ns).includes('qoder'))
              if (row === undefined) {
              ...
              // Read the field back from the document before answering: this is
              // the read-back check the client-side write path cannot perform on
              // its own, and it is what turns a "reported success" into "did
              // actually persist".
              const readBack = live()
