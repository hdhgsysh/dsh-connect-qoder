...
import { REGIONS, loadCredential, loadEnvCredential, sweepStaleOscryptDirs, setCredentialDiagnosticSink, isCredentialUsable } from './credentials.js'
...
class RegionRuntime {
  ...
  async resolveCredential() {
    ...
    // The two sources expire on different clocks (see `isCredentialUsable`):
    // an app credential by the `expired` flag its own store computed, an env
    // PAT by the wall clock on the job token it was exchanged for.
    if (isCredentialUsable(this.exchanged)) return this.exchanged
