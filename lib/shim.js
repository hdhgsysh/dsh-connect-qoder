...
export function createQoderShim(options) {
  ...
  async function chatCompletions(req, res) {
    ...
    try {
      ...
      const signInStale = error?.signInExpired === true || /sign-in is no longer valid|sign-in-expired/i.test(String(error?.message ?? ''))
      if (signInStale) {
        invalidateCredential?.()
      }
    ...
    try {
      ...
      // picked up without a DSH restart. The error carries `signInExpired`
      // when the upstream classifies the failure as sign-in-expired.
      if (error?.signInExpired === true || /sign-in is no longer valid|sign-in-expired/i.test(message)) {
        invalidateCredential?.()
      }
      ...
      // SSE error frame rather than a status-code change. The `event: error`
      // line sets the SSE event type; the `data:` frame carries the error object
      // in the OpenAI shape that pi-ai's parser inspects.
  ...
  return {
    ...
    token: () => SHARED_SECRET,
    close: () => {
      // Idempotent: repeated calls return the same promise so a double-close
      // (effect cleanup running twice under a React StrictMode-like re-mount)
      // does not call server.close() on an already-closed server.
      if (!closePromise) {
